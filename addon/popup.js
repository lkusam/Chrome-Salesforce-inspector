...
class App extends React.PureComponent {
  ...
  render() {
    let { sfHost, inDevConsole, inLightning, inInspector, addonVersion } = this.props;
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          inLightning &&
            !isInSetup &&
            h("a", { ref: "homeBtn", href: `https://${sfHost}/lightning/setup/SetupOneHome/home?setupApp=all`, title: "You can choose if you want to open in a new tab or not", target: linkTarget, className: "button" }, "Setup ", h("u", {}, "H"), "ome")
        )
      ),
      h(
        "div",
        ...
        h(
          "div",
          { className: "meta" },
          h("div", { className: "version" }, "(", h("a", { href: "https://github.com/sorenkrabbe/Chrome-Salesforce-inspector/blob/master/CHANGES.md" }, "v" + addonVersion), " / " + apiVersion + ")"),
...
class AllDataBox extends React.PureComponent {
  ...
  loadSobjects() {
    ...
    function addEntity({ name, label, keyPrefix }, api) {
      ...
      if (entity) {
        if (!entity.label) {
          // Doesn't seem to be needed, but if we have to do it for keyPrefix, we can just as well do it for label.
          entity.label = label;
        }
        if (!entity.keyPrefix) {
          // For some objects the keyPrefix is only available in some of the APIs.
  ...
  render() {
    ...
    return h(
      "div",
      { className: "all-data-box " + (this.isLoading() ? "loading " : "") },
      h(
        "ul",
...
class AllDataBoxUsers extends React.PureComponent {
  ...
  resultRender(matches, userQuery) {
    return matches.map((value) => ({
      ...
      element: [
        h(
          "div",
          { className: "autocomplete-item-main", key: "main" },
          h(MarkSubstring, {
        ...
        h(
          "div",
          { className: "autocomplete-item-sub small", key: "sub" },
  ...
  render() {
    ...
    return h(
      "div",
      { ref: "usersBox", className: "users-box" },
      ...
      h(
        ...
        selectedUser ? h(UserDetails, { user: selectedUser, sfHost, contextOrgId, currentUserId: contextUserId, linkTarget, contextPath }) : h("div", { className: "center" }, "No user details available")
      )
    );
  }
}
...
class AllDataBoxSObject extends React.PureComponent {
  ...
  loadRecordIdDetails() {
    ...
    if (selectedValue && selectedValue.recordId && selectedValue.sobject && selectedValue.sobject.availableApis && selectedValue.sobject.availableApis.includes("regularApi")) {
      ...
      sfConn
        .rest("/services/data/v" + apiVersion + "/query?q=" + encodeURIComponent(query), { logErrors: false })
        .then((res) => {
          for (let record of res.records) {
  ...
  resultRender(matches, userQuery) {
    return matches.map((value) => ({
      ...
      element: [
        ...
        h(
          "div",
          { className: "autocomplete-item-sub", key: "sub" },
          h(MarkSubstring, {
  ...
  render() {
    ...
    return h(
      ...
      selectedValue ? h(AllDataSelection, { ref: "allDataSelection", sfHost, showDetailsSupported, selectedValue, linkTarget, recordIdDetails, contextRecordId }) : h("div", { className: "all-data-box-inner empty" }, "No record to display")
    );
  }
}
...
class UserDetails extends React.PureComponent {
  ...
  render() {
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          h(
            "tbody",
            {},
            h("tr", {}, h("th", {}, "Name:"), h("td", {}, user.IsActive ? "" : h("span", { title: "User is inactive" }, "⚠ "), user.Name + " (" + user.Alias + ")")),
            h("tr", {}, h("th", {}, "Username:"), h("td", { className: "oneliner" }, user.Username)),
            h("tr", {}, h("th", {}, "E-mail:"), h("td", { className: "oneliner" }, user.Email)),
            h("tr", {}, h("th", {}, "Profile:"), h("td", { className: "oneliner" }, user.Profile ? h("a", { href: this.getProfileLink(user.ProfileId), target: linkTarget }, user.Profile.Name) : h("em", { className: "inactive" }, "unknown"))),
            h("tr", {}, h("th", {}, "Role:"), h("td", { className: "oneliner" }, user.UserRole ? user.UserRole.Name : "")),
            h(
              "tr",
              {},
              h("th", {}, "Language:"),
      ...
      h(
        "div",
        { ref: "userButtons", className: "center" },
...
class ShowDetailsButton extends React.PureComponent {
  ...
  onDetailsClick() {
    ...
    Promise.all([sfConn.rest(url), getAllFieldSetupLinks(sfHost, selectedValue.sobject.name)])
...
class AllDataSelection extends React.PureComponent {
  ...
  render() {
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          h(
            "tbody",
            {},
            h("tr", {}, h("th", {}, "Name:"), h("td", {}, h("a", { href: this.getObjectSetupLink(selectedValue.sobject.name), target: linkTarget }, selectedValue.sobject.name))),
            h("tr", {}, h("th", {}, "Label:"), h("td", {}, selectedValue.sobject.label)),
            h("tr", {}, h("th", {}, "Id:"), h("td", {}, h("span", {}, selectedValue.sobject.keyPrefix), h("span", {}, selectedValue.recordId ? " / " + selectedValue.recordId : "")))
          )
        ),
      ...
      selectedValue.recordId && selectedValue.recordId.startsWith("0Af") ? h("a", { href: this.getDeployStatusUrl(), target: linkTarget, className: "button" }, "Check Deploy Status") : null,
      buttons.map((button, index) =>
        h(
          ...
          button == "regularApi" ? "" : button == "toolingApi" ? " (Tooling API)" : " (Not readable)"
        )
      )
    );
  }
    ...
    if (recordIdDetails) {
      return h(
        ...
        h(
          "tbody",
          {},
          h("tr", {}, h("th", {}, "RecType:"), h("td", {}, recordIdDetails.recordTypeName)),
          h("tr", {}, h("th", {}, "Created:"), h("td", {}, recordIdDetails.created + " (" + recordIdDetails.createdBy + ")")),
          h("tr", {}, h("th", {}, "Edited:"), h("td", {}, recordIdDetails.lastModified + " (" + recordIdDetails.lastModifiedBy + ")"))
        )
      );
    } else {
      return null;
...
class AllDataSearch extends React.PureComponent {
  ...
  render() {
    ...
    return h(
      ...
      h("svg", { viewBox: "0 0 24 24", onClick: this.onAllDataArrowClick }, h("path", { d: "M3.8 6.5h16.4c.4 0 .8.6.4 1l-8 9.8c-.3.3-.9.3-1.2 0l-8-9.8c-.4-.4-.1-1 .4-1z" }))
    );
  }
}
...
function MarkSubstring({ text, start, length }) {
  ...
  return h("span", {}, text.substr(0, start), h("mark", {}, text.substr(start, length)), text.substr(start + length));
}
...
class Autocomplete extends React.PureComponent {
  ...
  render() {
    let { matchingResults } = this.props;
    let { showResults, selectedIndex, scrollTopIndex, itemHeight, resultsMouseIsDown } = this.state;
    ...
    return h(
      "div",
      { className: "autocomplete-container", style: { display: (showResults && matchingResults.length > 0) || resultsMouseIsDown ? "" : "none" }, onMouseDown: this.onResultsMouseDown, onMouseUp: this.onResultsMouseUp },
      h(
        "div",
        { className: "autocomplete", onScroll: this.onScroll, ref: "scrollBox" },
        h("div", { ref: "selectedItem", style: { position: "absolute", top: topSelected + "px", height: itemHeight + "px" } }),
        h("div", { style: { height: topSpace + "px" } }),
        matchingResults.slice(firstRenderedIndex, lastRenderedIndex + 1).map(({ key, value, element }, index) =>
          h(
            "a",
            {
              key,
